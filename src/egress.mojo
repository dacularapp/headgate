...
fn _redact_pii(owned payload: String) -> String:
...
struct EgressGuard(Movable):
    ...
    fn __init__(out self, owned fingerprints: List[String], owned canaries: List[String]):
        self.fingerprints = fingerprints^
        self.canaries = canaries^

    fn check(self, payload: String) raises -> String:
        """Return a redaction-scrubbed payload safe to send, or raise. Fails closed."""
        for ref c in self.canaries:
        ...
        for ref f in self.fingerprints:
